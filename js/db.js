...
function toDbCrew(c) {
  return {
    ...
    // skills & profile
    training: c.training || '', rating: c.rating || 0, skills: (() => { const s = { ...(c.skills || {}) }; if (c.tempAbbr && c.tempPosStart) { s._tempPromo = { posId: c.tempPosId, abbr: c.tempAbbr, start: c.tempPosStart, end: c.tempPosEnd || '' }; } else { delete s._tempPromo; } if (c.comments?.length) { s._comments = c.comments; } else { delete s._comments; } if (c.photo) { s._photo = c.photo; } else { delete s._photo; } return s; })(),
    // import / roster data
...
function fromDbCrew(r) {
  return {
    ...
    // skills & profile
    training: r.training || '', rating: r.rating || 0, skills: (() => { const s = { ...(r.skills || {}) }; delete s._tempPromo; delete s._comments; delete s._photo; return s; })(),
